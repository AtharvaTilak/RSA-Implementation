...
class RSA {
private:
	BigNumber p, q, n, phi, e, d, dec, cipher;
	string enc;
public:
...
public:
	...
	BigNumber getpubkey() {
		return rsa.getpublic();
	}
	void setn(Clients client) {
		rsa.setn(client.rsa.getn());
	}
	BigNumber getcipher() {
		return rsa.getcipher();
	}
	...
	void send(string m, BigNumber pubkey) {
		cout << "m: " << m << endl;
		rsa.encrypt(m);
		rsa.encode(pubkey);

	}
	void receive(BigNumber cipher) {
		rsa.decode(cipher);
		rsa.decrypt();
	}
};
...
int main() {
	...
	Alice.setn(Bob);
	Alice.setmessage(m);
	Alice.send(Alice.getmessage(), Bob.getpubkey());
	Bob.receive(Alice.getcipher());
	return 0;
}